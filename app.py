...
def email_action():
    ...
    try:
        ...
        if intent == "read_inbox":
            EMAIL_CACHE = fetch_emails(max_results=10) or []  # <-- (optional 10)
            CURRENT_INDEX = 0
            return jsonify({"intent": "read_inbox", "emails": EMAIL_CACHE, "index": CURRENT_INDEX})
        ...
        if intent == "next_email":
            ...
            return jsonify({"intent": "next_email", "email": EMAIL_CACHE[CURRENT_INDEX], "index": CURRENT_INDEX})

        if intent == "previous_email":
            if not EMAIL_CACHE:
                return jsonify({"error": "Inbox empty. Say 'read my inbox' first."}), 400
            CURRENT_INDEX = max(CURRENT_INDEX - 1, 0)
            return jsonify({"intent": "previous_email", "email": EMAIL_CACHE[CURRENT_INDEX], "index": CURRENT_INDEX})
        ...
        if intent == "read_email_number":
            ...
            return jsonify({"intent": "read_email_number", "email": EMAIL_CACHE[CURRENT_INDEX], "index": CURRENT_INDEX})
        ...
        if intent == "open_email":
            ...
            if not msg_id:
                return jsonify({"error": "No message id found for current email."}), 500  # <-- safety

            body = fetch_email_body(msg_id)
            return jsonify({"intent": "open_email", "email": EMAIL_CACHE[CURRENT_INDEX], "index": CURRENT_INDEX, "body": body})
        ...
        if intent == "reply_email":
            ...
            result = gmail_reply_email(
                ...
                current.get("subject", ""),
                message,
                thread_id=current.get("threadId")  # <-- IMPORTANT
            )
